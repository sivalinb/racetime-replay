...
def align(
    video: pd.DataFrame, workout: pd.DataFrame, offset_s: float = 0.0, tolerance_s: float = 2.0
) -> pd.DataFrame:
    ...
    for col in ["speed_mps", "heart_rate_bpm", "latitude", "longitude", "altitude_m"]:
        valid = workout[["elapsed_s", col]].dropna().sort_values("elapsed_s")
        if valid.empty:
            out[col] = np.nan
            continue
...
def summarize(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "interpretation": "Candidate stops and source disagreements require review. Camera motion is not running speed.",
    }
