...
### The problem
A watch can show a change in heart rate, training zone or pace. A camera can show what was visible. The two records usually live apart, leaving an athlete to search footage and compare timestamps manually. A harder section, a change in technique, a data gap and a camera pause can produce different stories. RaceTime Replay brings the evidence together before drawing conclusions.
...
### The broader concept
The general problem is understanding the situation around a recorded change. Ultra running is the implemented first case study; the concept may transfer to other settings, with different cameras, measurements and review requirements.

A proposed rehabilitation workflow would help a physiotherapist revisit a therapist-prescribed home task after a stroke or accident. With consent, a fixed camera could show the whole movement while an appropriate wearable records effort and timing. The clinician could review a pause, compare matched sessions and discuss fatigue during follow-up. Replay does not currently implement or clinically validate that workflow. It would need validated measurements, secure clinical handling and prospective evaluation, and would support professional judgment without diagnosing a condition or automatically changing treatment.

Workplace instruction and field inspections are other possibilities: connect a difficult step to its recorded context for a person to review. Wearable data should be collected only when it adds a defined benefit and with suitable consent; the sports schema is not automatically appropriate for those settings. See the [roadmap](ROADMAP.md#beyond-running).
