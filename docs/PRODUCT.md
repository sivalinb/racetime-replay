RaceTime Replay helps runners revisit the moments behind their workout numbers. Upload a first-person recording and workout data, then scrub through a shared timeline of video, speed, heart rate, route position, and events needing review.
...
The working prototype aligns a video with workout samples, measures visual motion with OpenCV, flags candidate stops and source disagreements, and lets a person review the supporting clip. A bounded agent retrieves relevant records and explains the measured events with references. The included synthetic demo makes the workflow reproducible without publishing anyone's private health or route data.
